...
Original file is located at
    https://colab.research.google.com/drive/1MQw0O2kzFae2jT25cbeYgXYjjJYmIfTw
...
class CbowDataset(data.Dataset):
    ...
    def make_training_examples(self):
        ...
        for sentence in self.sentences:
          for idx in range(self.context_size, (len(sentence) - self.context_size)):
            # prefix
            context = sentence[(idx - self.context_size):idx]
            # suffix
            context += sentence[idx + 1:(idx + 1 + self.context_size)]
            self.training_examples.append((context, sentence[idx]))
    ...
    def build_dictionaries(self, vocab):
        ...
        for idx in range(len(vocab)):
          self.idx2word[idx] = vocab[idx]
          self.word2idx[vocab[idx]] = idx
    ...
    def get_context_vector(self, idx):
        ...
        for word in context_words:
          context_indices.append(self.word2idx[word])
...
class CbowModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        relu = nn.ReLU()
        hidden = relu(self.linear(avg_embeds))
...
class TextDataset(data.Dataset):
    ...
    def build_dictionary(self):
        ...
        idx = 3
        for sentiment, review in self.examples:
          for word in review:
            ...
            if word_freq[word.lower()] == self.threshold:
              self.word2idx[word.lower()] = idx
              self.idx2word[idx] = word.lower()
              idx += 1
        ...
        for sentiment, review in self.examples:
          ...
          for word in review:
            if word.lower() in self.word2idx:
              index = self.word2idx[word.lower()]
            else:
              index = self.word2idx[CNN_UNK]
            review_indices.append(index)

          review_indices.append(self.word2idx[CNN_END])
    ...
    def get_text(self, idx):
        ...
        # return None
        review_indices = self.textual_ids[idx]
        if len(review_indices) < self.max_len:
          review_indices += [self.word2idx[CNN_PAD]] * (self.max_len - len(review_indices))
        else:
          review_indices = review_indices[:self.max_len]
        return torch.LongTensor(review_indices)
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
        ...
        # Define multiple Convolution layers (nn.Conv1d) with filter (kernel) size filter_height based on your
        ...
        self.convolution = nn.ModuleList([
          nn.Conv1d(embed_size, out_channels, filter_height, stride=stride)
        ...
        self.output = nn.Linear((out_channels * len(filter_heights)), num_classes)
    ...
    def forward(self, texts):
        ...
        # print(texts.shape) # 1, 10
        embedded = self.embedding(texts)
        # print("Shape of embedded tensor:", embedded.shape) # 1, 10, 16
        ...
        for conv in self.convolution:
          # tmp = conv(embedded.permute(0, 2, 1))
          # print("conv shape:", tmp.shape)
          cnn_out = F.relu(conv(embedded.permute(0, 2, 1)))
          # print("cnn_out shape:", cnn_out.shape)
          max_pooled = torch.max(cnn_out, dim=2)[0]
          cnn_outputs.append(max_pooled)

        concat_output = torch.cat(cnn_outputs, dim=1)
        ...
        return self.output(dropout_output)
