...
Original file is located at
    https://colab.research.google.com/drive/16VGljQ_J-I_6RP-eOQDvc-Hbx718aQSF
...
class RnnDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, hidden_units):
        ...
        self.attention_dec = nn.Linear(hidden_units, hidden_units)
        self.attention_enc = nn.Linear(hidden_units, hidden_units)
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        attention = self.attention_dec(dec_hs) + self.attention_enc(enc_output)
        scores = self.v_a(torch.tanh(attention))
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        rnn_input = torch.cat((context_vector.unsqueeze(1), x_embedded), dim=2)

        rnn_output, new_dec_hs = self.GRU(rnn_input, dec_hs)

        fc_out = self.output(rnn_output.squeeze(1))
...
def decode_rnn_model(encoder, decoder, src, max_decode_len, device):
    ...
    enc_output, enc_hs = encoder(src)

    dec_input, enc_hs, enc_output = dec_input.to(device), enc_hs.to(device), enc_output.to(device)
    dec_hidden_state = enc_hs

    for t in range (1, max_decode_len):
      pred_prob_un, dec_hidden_state, attent_weights = decoder(dec_input, dec_hidden_state, enc_output)
      curr_predictions[:,t,:] = pred_prob_un

      dec_input = torch.argmax(pred_prob_un, dim = 1).unsqueeze(1)
      curr_output[:, t] = dec_input.squeeze(1)
      # curr_output[:, t] = torch.argmax(pred_prob_un, dim = 1)
...
def create_positional_embedding(max_len, embed_dim):
    ...
    for i in range(embed_dim // 2):
      de = ((2 * i) / embed_dim) * np.log(10000)
      pe[pos, 2 * i] = np.sin(pos / np.exp(de))
      pe[pos, 2 * i + 1] = np.cos(pos / np.exp(de))
...
class TransformerEncoder(nn.Module):
    def __init__(self, src_vocab, embedding_dim, num_heads,
        ...
        enc_layer = nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=dim_feedforward).to(device)
        self.transformer_enc = nn.TransformerEncoder(enc_layer, num_layers=num_layers).to(device)
    ...
    def forward(self, x):
        ...
        emb = emb + self.positional_embedding[:x.size(0), :] #.unsqueeze(1)

        emb = self.dropout(emb)


        src_mask = self.make_src_mask(x) #.transpose(0, 1)

        output = self.transformer_enc(emb, src_key_padding_mask=src_mask)
...
class TransformerDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, num_heads,
        ...
        # num_layers, num_heads, & dim_feedforward here)
        decoder_layer = nn.TransformerDecoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=dim_feedforward).to(device)
        self.transformer_dec = nn.TransformerDecoder(decoder_layer, num_layers=num_layers).to(device)

        # Final fully connected layer
        self.output = nn.Linear(embedding_dim, trg_vocab_size).to(device)
    ...
    def forward(self, dec_in, enc_out):
        ...
        emb= self.embedding(dec_in).to(self.device)

        emb += self.position_embedding[:len(dec_in), :, :]
        emb = self.dropout(emb)

        target_mask = self.generate_square_subsequent_mask(len(dec_in))

        output = self.transformer_dec.forward(emb, memory=enc_out.to(self.device), tgt_mask=target_mask)
...
def decode_transformer_model(encoder, decoder, src, max_decode_len, device):
    ...
    for t in range(1, max_decode_len):
      dec_input = curr_output[:,:t].transpose(0, 1).to(device)
      pred_prob_un = decoder(dec_input.int(), enc_output)[-1]

      curr_predictions[:,t,:] = pred_prob_un

      best_token = torch.argmax(pred_prob_un, dim = 1)
      curr_output[:, t] = best_token
