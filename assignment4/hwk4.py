...
Original file is located at
    https://colab.research.google.com/drive/1_ou2yYuh008OuO6XuTPlyUrB1hyG-LBk
...
class ParserConfiguration(object):
    ...
    def parse_step(self, transition):
        ...
        if (transition == 'S'):
          ...
          trans_type = transition[:2]
          label = transition[3:]
          if (trans_type == 'LA'):
            w_j = self.stack.pop()
            w_i = self.stack.pop()
            self.stack.push(w_j)
            self.dependencies.add(w_j, w_i, label)
          elif (trans_type == 'RA'):
            w_j = self.stack.pop()
            w_i = self.stack.get_si(1)
            self.dependencies.add(w_i, w_j, label)
        # pass
...
def get_gold_action(stack, buffer, gold_dependencies):
    ...
    if (s2_arc.head == s1):
      return "LA-" + s2_arc.label
    elif (s1_arc.head == s2):
      has_s1_head = False
      for bi in buffer:
        bi_arc = gold_dependencies.getArcToHead(bi)
        if (bi_arc.head == s1):
          has_s1_head = True
          break
      if (has_s1_head == False):
        return "RA-" + s1_arc.label

    return "S" if len(buffer) != 0 else None
...
def generate_training_examples(sentence, gold_dependencies, vocab, feat_extract = lambda parser_config: []):
    ...
    stack, buffer = parser_config.stack, parser_config.buffer
    action = get_gold_action(stack, buffer, gold_dependencies)

    while (action != 'DONE'):
      if (action is None):
        return []
      features = feat_extract(parser_config)
      training_examples.append((features, action))
...
def get_top3_buffer_features(parser_config):
    ...
    items = min(len(buffer), 3)
    vocab = parser_config.vocab

    for i in range(items):
      bi = buffer.get_bi(i + 1)
      word_features[i] = bi.word_id
      pos_features[i] = bi.pos_id
...
def get_lc(parser_config, word):
    ...
    '''
    ### TODO ###
    dependencies = [arc for arc in parser_config.dependencies.arcs if (arc.head == word and arc.dependent.idx < word.idx)]
    return sorted(dependencies, key = lambda arc : arc.dependent.idx)
...
def get_rc(parser_config, word):
    ...
    ### TODO ###
    dependencies = [arc for arc in parser_config.dependencies.arcs if (arc.head == word and arc.dependent.idx > word.idx)]
    return sorted(dependencies, key = lambda arc : arc.dependent.idx, reverse = True)
...
def get_lc1_lc2_features(parser_config, i):
    ...
    items = min(2, len(lc))
    for i in range (items):
      word_features[i] = lc[i].dependent.word_id
      pos_features[i] = lc[i].dependent.pos_id
      label_features[i] = lc[i].label_id
...
def get_rc1_rc2_features(parser_config, i):
    ...
    items = min(2, len(rc))
    for i in range (items):
      word_features[i] = rc[i].dependent.word_id
      pos_features[i] = rc[i].dependent.pos_id
      label_features[i] = rc[i].label_id
...
def get_llc_rrc_features(parser_config, i):
    ...
    ### TODO ###
    si = parser_config.stack.get_si(i)
    lc1_si = get_lc(parser_config, si)
    rc1_si = get_rc(parser_config, si)


    if (len(lc1_si) > 0):
      # Get lc1 (lc1(si))
      lc = get_lc(parser_config, lc1_si[0].dependent)
      if (len(lc) > 0):
        word_features[0] = lc[0].dependent.word_id
        pos_features[0] = lc[0].dependent.pos_id
        label_features[0] = lc[0].label_id

    if (len(rc1_si) > 0):
      # Get rc1 (rc1(si))
      rc = get_rc(parser_config, rc1_si[0].dependent)
      if (len(rc) > 0):
        word_features[1] = rc[0].dependent.word_id
        pos_features[1] = rc[0].dependent.pos_id
        label_features[1] = rc[0].label_id
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embed_size, n_features, hidden_size, n_classes, dropout_prob):
        ...
        self.first_linear = nn.Linear(n_features * embed_size, hidden_size)
        ...
        self.second_linear = nn.Linear(hidden_size, n_classes)
    ...
    def forward(self, x):
        ...
        emb = self.embedding(x).reshape(batch_size, -1)
        output = relu(self.first_linear(emb))
...
def select_best_legal_action(parser_configs, predictions, n_labels):
    ...
    for i in range (len(parser_configs)):
      mask = np.ones_like(predictions[i])
      stack, buffer = parser_configs[i].stack, parser_configs[i].buffer
